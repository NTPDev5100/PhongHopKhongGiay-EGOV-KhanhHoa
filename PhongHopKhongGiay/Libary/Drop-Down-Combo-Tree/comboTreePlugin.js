...
;(function ( $, window, document, undefined ) {
    ...
    ComboTree.prototype.createFilterHTMLForMultiSelect = function (){
        return '<input id="' + this.comboTreeId + 'MultiFilter" type="text" class="multiplesFilter" placeholder="Nhập thành phần"/>';
    }
    ...
    ComboTree.prototype.createSourceItemHTML = function (sourceItem) {
        ...
        if (this.options.isMultiple)
            itemHtml += '<span data-id="' + sourceItem.id + '" data-selectable="' + isSelectable + '" class="comboTreeItemTitle ' + selectableClass + '">' + (!selectableLastNode && isSelectable ? '<input id="checkbox'+sourceItem.id+'" type="checkbox" />' : '') + sourceItem.title + '</span>';
        else
    ...
    ComboTree.prototype.setSelectionParent = function (selectionIdList) {
        if (selectionIdList && selectionIdList.length && selectionIdList.length > 0) {
            for (let i = 0; i < selectionIdList.length; i++) {
                let selectedItem = this.findItembyId(selectionIdList[i], this.options.source);
                ...
                if (selectedItem) {
                    ...
                    if (check) {
                        ...
                        if (!index) {
                            ...
                            this._selectedItems.push(selectedItem);
                            this._selectedItem = selectedItem;
                            $(selectedItemElem).find("#checkbox"+selectionIdList[i]+"").prop('checked', true);
                        }
                    }
                }
            }
